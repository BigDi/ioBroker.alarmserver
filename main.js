...
function onclientConnected(socket) {
  ...
  socket.on("data", function(data) {
    ...
    if (obj.Event == "MotionDetect") {
      adapter.log.debug("Motion : " + obj.Status);

      let queueDevice = createDevice(obj.SerialID);
      Promise.all(queueDevice)
        .then(() => {
...
function createDevice(serialNr) {
  const promises = [];
  promises.push(
    adapter.setObjectNotExistsAsync(serialNr, {
      type: "device",
      common: {
        name: serialNr,
        write: false,
...
function createChannel(serialNr, channel) {
  const promises = [];
  promises.push(
    adapter.setObjectNotExistsAsync(serialNr + "." + channel, {
      type: "channel",
      common: {
        name: channel,
        write: false,
